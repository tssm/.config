...
(defn- create-file [prompt-buffer-number]
	(procedures.create-file (aniseed.first (actions.get_selected_entry))))

(defn- edit-command [prompt-buffer-number]
...
(defn edit-first [prompt-buffer-number]
	...
	(match prompt-title
		"Command History" (edit-command prompt-buffer-number)
		"Find Files" (create-file prompt-buffer-number)
